...
<body>
    <div class="container">
        ...
        function input_data()
        {
            ...
            // Fetch the available Prodi data
            $prodi_query = "SELECT * FROM dt_prodi";
            $prodi_result = mysqli_query($hub, $prodi_query);
        
            // Default values for the Mahasiswa fields
            $row = array(
                "npm" => "",
                "nama" => "",
                "idprodi" => ""
            ...
            <form action="curd_mahasiswa.php?a=list" method="post" class="form-input">
                ...
                <?php
                // Create radio buttons for each Prodi
                while ($prodi_row = mysqli_fetch_array($prodi_result)) {
                    ?>
                    <input type="radio" name="idprodi" value="<?php echo $prodi_row['idprodi']; ?>" <?php if ($row["idprodi"] == $prodi_row['idprodi']) echo "checked"; ?>>
                    <?php echo $prodi_row['nmprodi']; ?>
                    <?php
                }
                ?>
        
                <!-- End of your input form fields -->
        ...
        function edit_data($id)
        {
            ...
            <form action="curd_mahasiswa.php?a=list" method="post" class="form-input">
                ...
                <input type="text" name="idprodi" id="idprodi" maxlength="11" size="11" value="<?php echo htmlspecialchars($row["idprodi"]); ?>" required /><br>
                <!-- End of your edit form fields -->
                <br><input type="submit" name="action" value="Simpan">
                <a href="curd_mahasiswa.php?a=list">Batal</a>
            </form>
            <?php
        ...
        function create_mahasiswa()
        {
            ...
            header("location: curd_mahasiswa.php?a=list");
            exit;
        }
